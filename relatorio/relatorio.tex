...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \texttt{isReachableIgnoringEdge(source,target,ignoreU,ignoreV)}
        \item \texttt{isEdgeRedundant(u,v)}
        \item \texttt{getEdgeListSnapshot()}
        \item \texttt{removeRedundantEdgesUsingSnapshot()}
    \end{itemize}
...
O \texttt{main} recebe um argumento obrigatorio (caminho do arquivo) e modos opcionais para consultas e reducao. O programa carrega o grafo e imprime:
\begin{itemize}
    ...
    \item quando \texttt{source target ignoreU ignoreV} sao informados: atingibilidade ignorando uma aresta especifica.
    \item quando \texttt{--reduce} e informado: reducao de arestas redundantes com snapshot das arestas originais.
\end{itemize}
...
\subsection{DFS ignorando aresta e reducao segura}
Foi implementada a variacao \texttt{isReachableIgnoringEdge(source,target,ignoreU,ignoreV)}. Durante a DFS, ao iterar vizinhos de um vertice \texttt{u}, a aresta \texttt{ignoreU -> ignoreV} e ignorada:
\begin{verbatim}
...
Com essa base, a funcao \texttt{isEdgeRedundant(u,v)} verifica se a aresta \texttt{u->v} e redundante.

Para evitar inconsistencias ao remover arestas enquanto se percorre a estrutura de adjacencia, foi implementada a estrategia de snapshot:
\begin{enumerate}
    \item \texttt{getEdgeListSnapshot()} cria uma copia fixa das arestas originais;
    \item \texttt{removeRedundantEdgesUsingSnapshot()} itera sobre essa copia;
    \item cada aresta redundante e removida do grafo atual com seguranca.
\end{enumerate}
...
\subsection{Consulta ignorando aresta}
A funcao \texttt{isReachableIgnoringEdge(source,target,ignoreU,ignoreV)} tem a mesma ordem assintotica da DFS padrao:
\[
O(n + m)
\]

\subsection{Reducao de arestas redundantes}
Na reducao, cada aresta original pode disparar uma DFS para verificar redundancia. Com $m$ arestas:
\[
O(m\cdot(n+m))
\]
no pior caso.
...
\subsection{Reducao completa com snapshot}
\begin{lstlisting}[style=cppstyle]
...
\begin{itemize}
    ...
    \item suporte a DFS ignorando aresta para detectar redundancia;
    \item reducao segura usando snapshot das arestas originais;
...
\begin{itemize}
    ...
    \item ja possui DFS e reducao por redundancia, mas ainda sem BFS/SCC/ordenacao topologica;
